xterm = [['0','Black (SYSTEM)','#000000','rgb(0,0,0)','hsl(0,0%,0%)'],
...
VIDEOS = [
    {
        ...
        "file": "x.sh",
        "json": "out-x.json",
        "framesRemoveFromEnd": 40
    },
    {
        "name": "space",
        "file": "space.sh",
        "json": "out.json",
        "framesRemoveFromEnd": 300
    }
]
for video in VIDEOS:
    ...
    with open(video['file']) as file:
        ...
        for line in file:
            ...
            # marker for first in frame
            if line[0:4] == "echo":
                if frameNumber != 0:
                    # print("Parsing frame #", frameNumber)
                    # shortern from both ends until matches height
                    # INEFFICIENT DONT CARE LUL
                    while len(current) > XQUESTHEIGHT:
                        del current[0]
                        if len(current) > XQUESTHEIGHT:
                ...
                # print("Start frame #", frameNumber)

                line = line[10:]

            frameLine = []
            for a in line.split(''): # tiny space here, not regular space
                if a[0:3] == "[38":
                    b = a[6:].split('m')
                    frameLine.append(b)
                    
            
            # shortern from both ends until matches width
            # INEFFICIENT DONT CARE LUL
            while len(frameLine) > XQUESTWIDTH:
                del frameLine[0]
                if len(frameLine) > XQUESTWIDTH:
                    del frameLine[len(frameLine) - 1]

            frameLineText = ""
            for char in frameLine:
                colorCode = char[0]
                displayChar = char[1][0]

                if video['name'] == "zoom down x" and displayChar == 's': 
                    displayChar = 'x'

                if colorCode == "16":
                    frameLineText = frameLineText + " "
                else:
                    if colorCode in colorChar:
                        frameLineText = frameLineText + colorChar[colorCode] + displayChar + " "
                    else:
                        colors.append(xterm[int(char[0])][2])
                        colorChar[char[0]] = str(len(colors) - 1)
                        frameLineText = frameLineText + colorChar[char[0]] + displayChar + " "

            frameLineText = frameLineText.rstrip()
            x_pos = len(frameLineText) - len(frameLineText.lstrip(" "))
            frameLineText = str(x_pos) + "|" + frameLineText.lstrip(" ")
            if frameLineText == "0|":
                frameLineText = ""
            current.append(frameLineText)
        file.close()


    with open(video['json'], "w") as out:
        frames = frames[:(-1 * video['framesRemoveFromEnd'])]
        buffer = '{{\n"colors": {},\n"frames": {}\n}}'.format(
            str(colors).replace("'", '"').replace(", ", ","),
            str(frames).replace("'", '"').replace(", ", ",")
        )
        out.write(buffer)
        out.close()
