...
Game.WindowHeight = 22;
Game.BaseSpeed = 60; //milliseconds
...
Game.Start = function() {
	...
	if (Game.Active == true) {
		...
		if ( Game.LevelLines > Game.GetLevelLines(Game.Level) - Game.WindowHeight) {
			Game.Warp = 0;
			Game.Invincible = 25;
	...
	if (Game.Mode == 'normal')
  {
	...
	for(i=start; i<end; i++) {
		if (Game.LineLength[i] != 0) {
			...
			Game.LineLength[i] -= 1;
			if (Game.LineLength[i] < getRandomInt(2*2, 2*6 - Math.floor(Game.Level/4)) && Game.LineReset[i] != 0) {
				Game.LineReset[i] = 0;
				if ((getRandomInt(1, 2) == 1 && i != end - 1) || i == start) {
					Game.LineReset[i+1] = 1;
					Game.LineLength[i+1] = getRandomInt(2*6, 2*22 - Game.Level);
				} else {
					Game.LineReset[i-1] = 1;
					Game.LineLength[i-1] = getRandomInt(2*6, 2*20 - Game.Level);
				}
			}
		}
	}
...
Game.AddLine = function() {
	var newMap = [];
	for (y = 1; y <= Game.WindowHeight; y++) {
		newMap[y-1] = Game.map[y];
	}
	Game.map = newMap;
	Game.map[Game.WindowHeight] = Game.GenerateLine();
	Game.LevelLines += 1;
